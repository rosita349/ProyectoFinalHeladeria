...
export const ModalCar = ({ isVisible, car, setShowModal, deleteCar}: Props) => {
    ...
    const totalItems = car.reduce((total, item) => total + item.totalQuantity, 0);
    let totalPrice = car.reduce((total, item) => total + (item.price * item.totalQuantity), 0);
